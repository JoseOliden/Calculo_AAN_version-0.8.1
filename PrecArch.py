...
def extract_and_tokenize_metadata(input_k0s_file, output_xlsx_file, num_lines):
    ...
    try:
        ...
        if not os.path.exists(input_k0s_file):
             ...
             st.error(f"Error: El archivo de entrada no se encontró: {input_k0s_file}")
             return None
        st.success(f"Leyendo las primeras {num_lines} líneas de: {input_k0s_file}...")
        ...
        st.success(f"Extracción de las primeras {num_lines} líneas exitosa.")
        st.success(f"Archivo de salida: **{output_xlsx_file}**")
...
def extraer_variables_clave(df):
    ...
    try:
        ...
        if df is not None and df.shape[0] > 5 and df.shape[1] > 1:
            ...
            st.error("Error: El DataFrame es demasiado pequeño o es None.")
